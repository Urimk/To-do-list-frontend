...
const Login = () => {
  ...
  const validateUsername = (username) => {
    ...
    if (
      !CHAR_REGEX.test(username) ||
      !LETTER_REGEX.test(username) ||
      SPECIAL_REGEX.test(username)
    )
  ...
  const handleSubmit = async (e) => {
    ...
    setBtnMessage("Signing In...");
    try {
      const res = await fetchData(details);
  ...
  const isBtnDisabled = btnMessage !== "Sign In";
